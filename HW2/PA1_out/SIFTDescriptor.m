...
%     img_dy = zeros(size(currentImage));
    h_x=[-1 0 1];
    h_y=[-1;0;1];
    img_dx=filter2(h_x,currentImage,'same');
    img_dy=filter2(h_y,currentImage,'same');
...
for i = 1 : N
    ...
    patch_mag_Sub=cell(0,1);
    patch_theta_Sub=cell(0,1);
    gridX = 1:num_histograms:patch_size+num_histograms;
    gridY = 1:num_histograms:patch_size+num_histograms;
    
    for p=1:patch_size/num_histograms
        for q=1:patch_size/num_histograms
            patch_mag_Sub=[patch_mag_Sub patch_mag(gridX(p):gridX(p+1)-1,gridY(q):gridY(q+1)-1)];
            patch_theta_Sub=[patch_theta_Sub patch_theta(gridX(p):gridX(p+1)-1,gridY(q):gridY(q+1)-1)];
        end
    end
    
    feature=zeros(0,1);
    for n=1:num_histograms * num_histograms
        [histogram, ~] = ComputeGradientHistogram(num_angles, patch_mag_Sub{n}, patch_theta_Sub{n});
        feature = [feature, histogram];
    end
...
for i=1:num_bins
    low_angles=(i-1)*angle_step;
    high_angles=i*angle_step;
    logic_angle_matrix = (gradient_angles >= low_angles) & (gradient_angles <= high_angles);
    %the sum of entries in gradient_magnitudes whose corresponding
%       gradient_angles lie between 0 and angle_step.
    histogram(i)=sum(sum(gradient_magnitudes.*logic_angle_matrix));
...
direction=angles(histogram==max(histogram));
